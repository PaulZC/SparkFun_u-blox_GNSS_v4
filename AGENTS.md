...
The code I shared with you from `C:\Users\pc235\Documents\SparkX\SparkFun_u-blox_GNSS_v4` includes ```numQueriedWords```. E.g. ```const uint8_t numQueriedWords = 2;   // We need this many words (uint32_t) to hold the queried flags```. Again, do not use this approach. Use a single ```bool``` to indicate if the whole message is fresh or stale.
